...
serve(async (req) => {
  ...
  try {
    ...
    const htmlBody = buildEmail({
      ...
      preheader: (from_name || "Someone") + " sent you a pitch" + (workspaceName ? " from " + workspaceName : ""),
      heading: (from_name || "Someone") + " sent you a pitch" + (workspaceName ? " from " + workspaceName : ""),
      body: bodyContent,
