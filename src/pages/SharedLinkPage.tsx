...
import { Lock, Play, Pause, Volume2, VolumeX, Music, AlertCircle, Clock, Disc3, Download } from "lucide-react";
...
export default function SharedLinkPage() {
  ...
  useEffect(function() {
    ...
    async function fetchLink() {
      ...
      if (link.track_id) {
        var { data: track, error: trackErr } = await supabase
          .from("tracks")
  ...
  var togglePlay = useCallback(function() {
    ...
    if (!audio || !trackData?.audio_url) return;

    if (isPlaying) {
      audio.pause();
      setIsPlaying(false);
    } else {
      if (!audio.src || audio.src !== trackData.audio_url) {
            ...
            <div className="flex items-start gap-5 p-6">
              ...
              <div className="min-w-0 flex-1 pt-1">
                <p className="text-[10px] uppercase tracking-wider text-primary font-semibold mb-1">
                  {linkData?.share_type === "stems" ? "Stems" : linkData?.share_type === "playlist" ? "Playlist" : linkData?.share_type === "pack" ? "Pack" : "Track"}
                </p>
