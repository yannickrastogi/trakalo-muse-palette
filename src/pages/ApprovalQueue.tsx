...
const statusConfig: Record<SendStatus, { label: string; color: string; icon: typeof Clock }> = {
  draft: { label: "Draft", color: "bg-muted-foreground/12 text-muted-foreground", icon: FileText },
  pending_approval: { label: "Pending", color: "bg-brand-orange/12 text-brand-orange", icon: Clock },
  approved: { label: "Approved", color: "bg-emerald-500/12 text-emerald-400", icon: CheckCircle2 },
  rejected: { label: "Rejected", color: "bg-destructive/12 text-destructive", icon: XCircle },
  sent: { label: "Sent", color: "bg-emerald-500/12 text-emerald-400", icon: Send },
  cancelled: { label: "Cancelled", color: "bg-muted-foreground/12 text-muted-foreground", icon: XCircle },
};
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="p-4 sm:p-6 lg:p-8 max-w-[900px] space-y-6">
          <motion.div variants={item} className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Approval request details</p>
            </div>
          ...
          <motion.div variants={item} className="card-premium p-5 space-y-4">
            <div className="grid grid-cols-2 gap-4 text-[13px]">
              <div>
                <p className="text-2xs text-muted-foreground font-medium uppercase tracking-wider mb-1">Send Type</p>
                <div className="flex items-center gap-1.5">
                  <TypeIcon className="w-4 h-4 text-brand-orange" />
                  <span className="font-semibold text-foreground">{sendTypeLabels[selectedSend.sendType]}</span>
                </div>
              </div>
              <div>
                <p className="text-2xs text-muted-foreground font-medium uppercase tracking-wider mb-1">Team</p>
                <span className="font-semibold text-foreground">{team?.name || "—"}</span>
              </div>
              <div>
                <p className="text-2xs text-muted-foreground font-medium uppercase tracking-wider mb-1">Created By</p>
                <div className="flex items-center gap-1.5">
              ...
              <div>
                <p className="text-2xs text-muted-foreground font-medium uppercase tracking-wider mb-1">Recipient(s)</p>
                <span className="font-semibold text-foreground">{selectedSend.recipients}</span>
              </div>
              <div className="col-span-2">
                <p className="text-2xs text-muted-foreground font-medium uppercase tracking-wider mb-1">Created</p>
            ...
            {selectedSend.message && (
              <div>
                <p className="text-2xs text-muted-foreground font-medium uppercase tracking-wider mb-1">Message</p>
          ...
          {selectedSend.status === "pending_approval" && (
            <motion.div variants={item} className="space-y-3">
              {/* Internal Note */}
              <div>
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Internal Note (optional)</label>
                <Textarea
                  value={internalNote}
              ...
              {showRejectForm && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}>
                  <label className="text-xs font-semibold text-destructive uppercase tracking-wider">Rejection Reason (optional)</label>
                  <Textarea
                    value={rejectReason}
                    onChange={(e) => setRejectReason(e.target.value)}
                    placeholder="Explain why this send is being rejected..."
          ...
          <motion.div variants={item} className="card-premium overflow-hidden">
            <div className="px-5 py-3 border-b border-border flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-foreground">Approval History</h3>
            </div>
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="p-4 sm:p-6 lg:p-8 max-w-[1200px] space-y-6">
        ...
        <motion.div variants={item} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-foreground tracking-tight">Approval Queue</h1>
              <p className="text-muted-foreground text-xs sm:text-sm mt-0.5">Review and manage outbound send requests</p>
            </div>
          </div>
          {pendingCount > 0 && (
        ...
        {filteredSends.length === 0 ? (
          <motion.div variants={item} className="card-premium p-12 text-center">
            ...
            <p className="text-sm text-muted-foreground font-medium">
              {filter === "pending" ? "No pending approvals" : "No sends found"}
            </p>
            <p className="text-xs text-muted-foreground/60 mt-1">
              {filter === "pending" ? "All outbound sends have been reviewed" : "Try adjusting your filters"}
            </p>
          </motion.div>
        ) : (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="text-2xs shrink-0 border-border">{sendTypeLabels[send.sendType]}</Badge>
                    </div>
